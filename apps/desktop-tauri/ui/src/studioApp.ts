...
`;
  ...
  function updatePreviewForActiveClip(activeClips: unknown[]) {
    ...
    const activeClip = activeClips[0] as { id?: number; asset_id?: string | number; src_in_tick?: number };

    const sourceClip = timelineState.tracks
      .flatMap((t) => t.clips)
      .find((c) => c.id === activeClip.id);

    const asset = registeredAssets.find((a) => String(a.id) === String(sourceClip?.assetId));

    if (!asset) {
      preview.innerHTML = `<div class="preview-overlay"><span>Active clip has no linked asset</span></div>`;
      return;
    }

    if (asset.meta_jsonb?.proxy_status !== "ready" || !asset.meta_jsonb?.proxy_path) {
      preview.innerHTML = `<div class="preview-overlay"><span>Preview still processing</span></div>`;
      return;
    }

    const src = `${ORCH_BASE}/v1/media/stream?path=${encodeURIComponent(asset.meta_jsonb.proxy_path)}`;
    const timeSec = Math.max(
      0,
      ((timelineState.playheadTick - (activeClip.src_in_tick ?? 0)) / timelineState.fps)
    );

    preview.innerHTML = `<video id="preview-video" muted playsinline></video>`;
  ...
  function renderAssetList() {
    ...
    for (const asset of registeredAssets) {
      ...
      li.addEventListener("click", () => {
        ...
        const serverAssetId = nextClipId + 9000;
        const nextClip: Clip = {
          id: nextClipId++,
          assetId: asset.id,
          serverAssetId,
        ...
        sequence: {
          ...
          clips: timelineState.tracks.flatMap((t) =>
            t.clips.map((c) => ({
              id: c.id,
              track_id: t.id,
              asset_id: c.serverAssetId ?? c.id + 9000,
        ...
        for (const clip of track.clips) {
          if (!clip.serverId && track.serverId) {
            const c = await orchestratorCreateClip(
              activeSequenceId,
              track.serverId,
              clip.serverAssetId ?? clip.id + 9000,
              clip.inTick,
              clip.outTick
            );
            clip.serverId = c.id;
