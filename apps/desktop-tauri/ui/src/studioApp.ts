...
document.querySelector("#btn-new-project")!.addEventListener("click", () => {
  ...
  const projectNameInput = document.querySelector<HTMLInputElement>("#project-name");
  if (projectNameInput) projectNameInput.value = `Untitled ${new Date().toLocaleTimeString()}`;
  ...
  writeOutput({
    action: "new_project",
    status: "ready",
    note: "Timeline reset to start; add assets by clicking in explorer.",
  });
  renderTimeline();
});

document.querySelector("#btn-save-project")!.addEventListener("click", () => {
  saveProjectToStorage();
});

document.querySelector("#btn-load-project")!.addEventListener("click", () => {
