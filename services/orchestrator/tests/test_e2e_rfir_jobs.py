...
Only the two ML ops (t2i, depth) are faked via the shared fake_ml_ops
fixture (conftest.py) — HTTP layer, guardrails, job store, worker thread,
bridge, compiler, validator, executor engine, and ffmpeg are all real.
The failure path runs with NO fakes at all: torch isn't installed in this
venv, so the t2i op genuinely fails, which must land the job in "failed"
